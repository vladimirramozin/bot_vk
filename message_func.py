...
keyboard_for_main.add_button('о нашей команде', color=VkKeyboardColor.SECONDARY)
...
#keyboard_for_shop.add_line()
#keyboard_for_shop.add_button('назад', color=VkKeyboardColor.PRIMARY)
...
def preparing_images_for_sending(shop_category, vk, connection_bd, upload, user_id, keyboard=None):
    select_shop = "SELECT products, photo FROM shop WHERE category = {}".format(shop_category)
    shop = execute_read_query(connection_bd, select_shop)
    try:
        for photo in shop:
            attachmets = []
            uploade_image = upload.photo_messages(photos= photo[1])[0]
            attachmets.append('photo{}_{}'.format(uploade_image['owner_id'], uploade_image['id']))
            #pdb.set_trace()
            write_msg(user_id, f'{photo[0]}', vk, attachmets, keyboard)
...
def write_msg(user_id, message, vk, attachment=None, keyboard=None):
    if keyboard == 'keyboard_for_shop':
        return vk.method('messages.send', {'user_id': user_id, 'message': message, 'random_id': get_random_id(), 'keyboard':keyboard_for_shop.get_keyboard()})
    #elif keyboard == 'back':
    #    pdb.set_trace()
    #    return vk.method('messages.send', {'user_id': user_id, 'message': message, 'random_id': get_random_id(),
    #                                'keyboard': keyboard_for_shop.get_keyboard()})
    elif keyboard == 'team':
        return vk.method('messages.send', {'user_id': user_id, 'message': message, 'random_id': get_random_id(), 'attachment': ','.join(attachment),
        ...
        return vk.method('messages.send', {'user_id': user_id, 'message': message, 'random_id': get_random_id(), 'attachment': ','.join(attachment), 'keyboard':keyboard_for_main.get_keyboard()})
    else:
        return  vk.method('messages.send', {'user_id': user_id, 'message': message, 'random_id': get_random_id()})
